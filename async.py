from typing import Callable, Any, List, Union, Sequence, Tuple, Set
import functools
import time
import asyncio
import logging
...
def decor(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    async def timekeeping_async(*args: Any, **kwargs: Any) -> Any:
        ...
        logger.info(f'Обработка запроса для {meta_func(func, *args, **kwargs)} заняла {time.time() - start_time:.2f} сек')
...
async def async_gather() ->  Sequence[Any]:
    """Асинхронная основная функция для выполнения запросов при помощи asyncio.gather().
       ...
       """
    result= await asyncio.gather(exemple_async('Москва'), exemple_async(), exemple_async('Уфа'),
...
async def async_task()-> Union[str, None]:
    ...
    logger.info(f'Состояние задачи №1 {task_1.done()} | Состояние задачи №2 {task_2.done()} | Состояние задачи №4 {task_4.done()}')
    res2 = await task_2
    ...
    if task_4.done():
        res4 = task_4.result()
        print(f'Результат работы задачи №4 {res4}')
        return print(f'РЕЗУЛЬТАТЫ РАБОТЫ ФУНКЦИИ {__name__} ЗАДАЧА№1 {res1}, ЗАДАЧА№2 {res2}, ЗАДАЧА№4 {res4}')

#---as_completed---

@decor
async def as_complete_main() -> None:
    tasks = [exemple_async('Москва'),
             exemple_async('Уфа'),
             # exemple_async(),
             exemple_async('Сочи')]
